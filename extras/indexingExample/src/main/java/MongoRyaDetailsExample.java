...
public class MongoRyaDetailsExample {
    ...
    public static void main(final String[] args) throws Exception {
        ...
        try {
            ...
            testSecondaryIndexerExists(sail);
        } finally {
    ...
    private static void testSecondaryIndexerExists(final Sail sail) throws RyaDAOException, MalformedQueryException, RepositoryException, TupleQueryResultHandlerException, QueryEvaluationException, AccumuloException, AccumuloSecurityException, IOException {
        final RdfCloudTripleStore rdfConn = (RdfCloudTripleStore)sail;
        final MongoDBRyaDAO dao = (MongoDBRyaDAO) rdfConn.getRyaDAO();
        final MongoClient client = dao.getMongoClient();
        final DB instanceDB = client.getDB(INSTANCE);
        final DBCollection ryaColl = instanceDB.getCollection("rya");
        boolean hasIndex = false;
        for(final DBObject dbo : ryaColl.getIndexInfo()) {
            if(dbo.get("name").equals("text_text") ) {
                hasIndex = true;
                break;
            }
        }
        Validate.isTrue(hasIndex);
    }
    ...
    private static Configuration getConf() {
        ...
        conf.set(MongoDBRdfConfiguration.MONGO_DB_NAME, INSTANCE);
