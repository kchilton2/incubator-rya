...
public class ConnectorFactoryIT extends AccumuloITBase {
    ...
    public void connect_successful() throws AccumuloException, AccumuloSecurityException {
        ...
        final CharSequence password = CharBuffer.wrap( getConnectionDetails().getPassword() );

        final ConnectorFactory ac = new ConnectorFactory();
        ac.connect(getConnectionDetails().getUsername(),
                password,
                getConnectionDetails().getInstanceName(),
                getConnectionDetails().getZookeepers());
    }
