...
public class RyaDetailsToConfiguration {
    ...
    public static final String CONF_PCJ_IDS = "pcj.index.ids";
    /**
    ...
    public static void addRyaDetailsToConfiguration(final RyaDetails details, final Configuration conf) {
        ...
        final boolean pcjEnabled = details.getPCJIndexDetails().isEnabled();
        checkAndSet(conf, ConfigurationFields.USE_PCJ, pcjEnabled);
        if(pcjEnabled) {
            final PCJIndexDetails pcjDetails = details.getPCJIndexDetails();
            final List<String> pcjIds = new ArrayList<>();
            for(final PCJDetails pcj : pcjDetails.getPCJDetails()) {
                pcjIds.add(pcj.getId());
            }
            conf.setStrings(CONF_PCJ_IDS, pcjIds.toArray(new String[]{}));
        }
    }
